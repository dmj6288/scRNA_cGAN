...
dataloader = DataLoader(
    MNIST(
        "/home/dennis00/scRNA_GAN/MNIST", download=False, transform=transform
    ),
...
for epoch in range(n_epochs):
    ...
    for real, labels in tqdm(dataloader):
        ...
        if cur_step % display_step == 0 and cur_step > 0:
            ...
            plot_images_from_tensor(fake, epoch, "FAKE")
            plot_images_from_tensor(real, epoch, "REAL")
            ...
            plt.legend()
            plt.show()
            plt.savefig("/home/dennis00/scRNA_GAN/cGAN_exports/loss_plots/" + str(epoch) + ".png", dpi=300, bbox_inches='tight')

            # Define the checkpoint dictionary

            checkpoint = {'generator_state_dict':     gen.state_dict(),  'generator_optimizer_state_dict': gen_opt.state_dict(),
                          'discriminator_state_dict': disc.state_dict(), 'discriminator_optimizer_state_dict': disc_opt.state_dict()}

	    # Save the checkpoint to a file
            torch.save(checkpoint, "/home/dennis00/scRNA_GAN/cGAN_exports/checkpoints/model_checkpoint_" + str(epoch) + ".pth")
        elif cur_step == 0:
